...
public class FirstFollow_10 {
	...
	public static void main(String[] args) {
		...
		String firstEncoding = First(input);
		String followEncoding = Follow(input);
	...
	private static String GenerateFirst(Rule R, String[][] Elkbeer, int i, int max) {
		...
		for (int j = 1; j < max; j++) {
			...
			if (word != null) {
				...
				if (word.charAt(0) >= 65 && word.charAt(0) <= 90) { // if first letter is a Capital letter
					if (!((word.charAt(0) + "").equals(letter))) {
						
						for (int j2 = 0; j2 < RA.size(); j2++) { //looping on RA to bring the first of the Capital letter or Generate it 
							
							if ((RA.get(j2).getLetter().equals(word.charAt(0) + ""))) { // last edit
						...
						int Counter=0;
						if(word.length()-1!=Counter) { //if there are still letters to follow
						Counter++;
	...
	private static String Follow(String input) {
		input = input.replaceAll("\\s+", "");
		// System.out.println(input);
							...
							if (m != word.length() - 1) { // if not last letter
								if (word.charAt(m + 1) >= 97 && word.charAt(m + 1) <= 122) // Case 1 - Small letter
									...
									for (int n = 0; n < RA.size(); n++) {
										...
										// Add here code to get the letter after if the string obtained contains epsilon
									}
								}
